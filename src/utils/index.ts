  ...
  if (response.transactions && response.total) {
    response.transactions.forEach(async (t: Transaction) => {
      ...
      const amountFormated = `${formatNumber(fromWei(t.amount, t.originDecimals).toNumber())} ${
        t.originToken == NATIVE_TOKEN_ADDRESS ? originNetwork.nativeCurrency.symbol : t.originSymbol
      }`
      ...
      transactions.push({
        ...t,
        tokenSymbol: t.originToken == NATIVE_TOKEN_ADDRESS ? originNetwork.nativeCurrency.symbol : t.originSymbol,
        fromNetwork,
        toNetwork,
        originNetwork,
        amountFormated,
